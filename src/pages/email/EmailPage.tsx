...
export function EmailPage() {
  ...
  const [section, setSection] = useState<'campaigns' | 'templates' | 'settings'>('campaigns');
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1 shadow-panel" role="tablist" aria-label="Seções de e-mail">
        {([['campaigns', `Campanhas (${campaigns.length})`], ['templates', `Templates (${templates.length})`], ['settings', 'Configuração']] as const).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={section === value} className={`min-h-10 shrink-0 rounded-lg px-4 text-sm font-bold ${section === value ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'}`} onClick={() => setSection(value)}>{label}</button>)}
      </div>

      {section === 'settings' && <div className="max-w-3xl">
        <Card>
