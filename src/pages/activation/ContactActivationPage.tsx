    ...
    <div className="space-y-5">
      ...
      {loading ? <LoadingState /> : summary && (
        <>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <select className="h-11 w-full rounded-md border border-slate-200 bg-white px-3 text-sm" value={selectedStudentId} onChange={(event) => { setSelectedStudentId(event.target.value); setAssisted(undefined); }}>
                  ...
                  {students.map((student) => <option key={student.id} value={student.id}>{student.name} · {sourceLabel(student.source)}</option>)}
                </select>
          ...
          <Card>
            <CardHeader><h2 className="font-bold text-slate-950">Vínculos que precisam de revisão</h2><p className="mt-1 text-sm text-slate-500">Acontece quando nome, origem e presença não produziram uma correspondência única.</p></CardHeader>
            ...
            <CardContent className="space-y-2">
              {recent.length === 0 ? <EmptyState message="Nenhuma ativação iniciada." /> : recent.map((item) => (
                <div key={item.id} className="flex flex-col gap-2 rounded-lg border border-slate-100 px-3 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="font-semibold text-slate-900">{item.student_name || item.claimed_name}</p><p className="text-xs text-slate-500">{item.is_new_student ? 'Novo cadastro · ' : ''}{sourceLabel(item.source)} · {new Date(item.created_at).toLocaleString('pt-BR')}</p></div>
