...
export function PublicContactActivationPage({ code }: { code: string }) {
  ...
  if (loading) {
    return <main className="flex min-h-screen items-center justify-center bg-slate-50 p-6"><LoadingState label="Abrindo sua ativação" /></main>;
  }

  return (
    <main className="min-h-screen bg-gradient-to-b from-emerald-50 via-white to-slate-50 px-4 py-8 sm:py-14">
      <div className="mx-auto max-w-lg">
        <div className="mb-6 text-center">
          ...
          <p className="text-xs font-bold uppercase tracking-widest text-emerald-700">EngageFit</p>
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">{config?.box_name || 'Ativação no WhatsApp'}</h1>
          <p className="mt-2 text-sm leading-6 text-slate-600">Acompanhe seus check-ins, metas e brindes diretamente pelo WhatsApp.</p>
        </div>
          ...
          {result ? (
            ...
            <form className="space-y-5" onSubmit={submit}>
              ...
              <fieldset>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {(['wellhub', 'totalpass'] as Source[]).map((item) => (
                    <label key={item} className={`cursor-pointer rounded-lg border p-3 text-center text-sm font-bold transition ${source === item ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-slate-200 text-slate-600'}`}>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
                <input className="mt-1 h-4 w-4 accent-emerald-600" type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} required />
              ...
              <div className="flex items-start gap-2 text-xs leading-5 text-slate-500"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Seu telefone não vem do Wellhub ou TotalPass. Ele será recebido somente quando você enviar a confirmação pelo WhatsApp.</div>
            </form>
          ) : null}
        </section>
      </div>
